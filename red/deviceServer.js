...
devServer.prototype.start = function() {
    ...
    //wsServer.start();
    //discovery.start();
    var connectionPool = require("../nodes/core/io/lib/mqttConnectionPool");
    ...
    client.subscribe(settings.masterDevice,2,function(topic,payload,qos,retain) {
        if (settings.masterDevice === settings.deviceId)
            return;
        
        // download flows
        var nt = typeRegistry.get("http request");
        var nn = new nt({"url":settings.masterDevice + "flows"});
        var result = {};
        nn.receive(result);
